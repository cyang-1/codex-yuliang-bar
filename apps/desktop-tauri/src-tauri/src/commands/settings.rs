...
impl SettingsUpdate {
    ...
    fn apply_to(self, settings: &mut Settings) -> Result<crate::floatbar::SettingsPatch, String> {
        let requested_float_bar_patch = self.float_bar_patch();
        ...
        requested_float_bar_patch.apply(settings);
        settings.apply_codex_only_mode();
        Ok(crate::floatbar::SettingsPatch {
            enabled: Some(settings.float_bar_enabled),
            opacity: Some(settings.float_bar_opacity),
            scale: Some(settings.float_bar_scale),
            orientation: Some(settings.float_bar_orientation.clone()),
            style: Some(settings.float_bar_style.clone()),
            click_through: Some(settings.float_bar_click_through),
            provider_ids: Some(settings.float_bar_provider_ids.clone()),
            dark_text: Some(settings.float_bar_dark_text),
            show_reset_inline: Some(settings.float_bar_show_reset_inline),
        })
    }
}
