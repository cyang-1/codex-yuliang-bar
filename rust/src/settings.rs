...
impl Default for Settings {
    fn default() -> Self {
        ...
        enabled.insert(CODEX_PROVIDER_ID.to_string());
        ...
        Self {
            ...
            merge_tray_icons: true, // Codex-only fork uses one tray surface.
            ...
            show_as_used: false,       // Match Codex Desktop's remaining quota display
            enable_animations: true,   // Animations enabled by default
            reset_time_relative: false, // Match Codex Desktop's absolute reset time display
            ...
            float_bar_provider_ids: vec![CODEX_PROVIDER_ID.to_string()],
...
impl Settings {
    ...
    pub fn settings_path() -> Option<PathBuf> {
        dirs::config_dir().map(|p| p.join("CodexOnlyBar").join("settings.json"))
    }
    ...
    pub fn load() -> Self {
        ...
        settings.apply_codex_only_mode();
        settings
    }
    ...
    /// Keep this fork focused on Codex even when it reads a settings file
    /// created by the full multi-provider CodexBar app.
    pub fn apply_codex_only_mode(&mut self) {
        self.enabled_providers.clear();
        self.enabled_providers
            .insert(CODEX_PROVIDER_ID.to_string());
        self.float_bar_enabled = true;
        self.float_bar_provider_ids = vec![CODEX_PROVIDER_ID.to_string()];
        self.float_bar_orientation = "vertical".to_string();
        self.float_bar_style = "floating".to_string();
        self.float_bar_scale = 75;
        self.float_bar_dark_text = true;
        self.float_bar_show_reset_inline = true;
        self.show_as_used = false;
        self.reset_time_relative = false;
        self.ui_language = Language::Chinese;
        self.merge_tray_icons = true;
        self.provider_order = vec![CODEX_PROVIDER_ID.to_string()];
        self.provider_metrics.retain(|id, _| id == CODEX_PROVIDER_ID);
        self.provider_configs
            .retain(|provider, _| *provider == ProviderId::Codex);
    }
